...
function gameOver() {
	cancelAnimationFrame(animationId);
	clearInterval(timer);
	const hitSoundEffect = new Audio("Media/hit-sound-effect.wav");
	...
	setTimeout(() => {
		...
		endScoreEl.innerHTML = score;
		startGameContainer.style.display = "flex";
		const swooshSoundEffect = new Audio("Media/swoosh-sound-effect.wav");
		swooshSoundEffect.volume = 0.2;
		swooshSoundEffect.play();
		if (player.y + player.radius < canvas.height) {
			const dieSoundEffect = new Audio("Media/die-sound-effect.wav");
...
function animate(currentTime) {
	...
	pipes.forEach((pipe, index) => {
		...
		if (!pipe.isPassed && player.x > pipe.x + pipe.width) {
			...
			const scoreSoundEffect = new Audio("Media/point-sound-effect.wav");
		...
		if (
			Math.hypot(player.x - pointOnRect.x, player.y - pointOnRect.y) -
				player.radius <=
			0
		) {
			gameOver();
...
addEventListener("click", () => {
	if (player.isAlive) {
		...
		const flapSoundEffect = new Audio("Media/flap-sound-effect.wav");
